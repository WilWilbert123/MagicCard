...
export default function HrAuditLogsPage() {
  ...
  const fetchAuditLogs = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/audit-logs?type=${selectedType}`);
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Timestamp', 'Actor', 'Actor Type', 'Security Action', 'Target Entity', 'Details', 'IP Address'];
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="p-4 rounded-xl bg-white dark:bg-[#111827]/90 border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        {/* Type Tabs */}
        <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900 p-1 rounded-lg border border-slate-200/80 dark:border-slate-800 text-xs w-full sm:w-auto">
