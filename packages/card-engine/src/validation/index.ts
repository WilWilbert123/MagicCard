 ...
 */
export function validateCardTemplate(template: CardTemplateJSON): ValidationResult {
  ...
  const frontHasName = template.front.elements.some(
    (el) => el.type === 'TEXT' && (el.text.includes('{{employee.fullName}}') || el.text.includes('{{employee.firstName}}'))
  );
