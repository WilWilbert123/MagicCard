import { resolveDataBinding } from '../resolver';
 ...
 */
export async function renderCardToCanvas(canvas, template, side, employee, options = {}) {
...
function drawText(ctx, el, employee, baseUrl) {
...
async function drawImage(ctx, el, employee, baseUrl, signal) {
    ...
    await drawImageFromUrl(ctx, resolvedSrc || el.src, el.x, el.y, el.width, el.height, el.borderRadius, el.borderWidth, el.borderColor, signal, el.objectFit, el.tintColor);
}
